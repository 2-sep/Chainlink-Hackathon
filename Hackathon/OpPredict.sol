 ...
 * @title EggCrowns
 */
contract OpEggCrowns is Pausable, ReentrancyGuard, OwnerIsCreator {
    ...
    function predictBear(
        ...
        _sendMessageBear(epoch, msg.sender, amount);
    ...
    function predictBull(
        ...
        _sendMessageBull(epoch, msg.sender, amount);
    ...
    function updateResult(
        ...
        emit UpdateResult(epoch, rewardAmount, rewardBaseCalAmount);
    }
    ...
    function _sendMessageBear(
        uint256 epoch,
        address user,
        uint256 amount
    ) internal returns (bytes32 messageId) {
        bytes memory functionCall = abi.encodeWithSignature(
            "CCIPpredictBear(uint256,address,uint256)",
            epoch,
            user,
            amount
        );
    ...
    function _sendMessageBull(
        uint256 epoch,
        address user,
        uint256 amount
    ) internal returns (bytes32 messageId) {
        bytes memory functionCall = abi.encodeWithSignature(
            "CCIPpredictBull(uint256,address,uint256)",
            epoch,
            user,
            amount
        );
